...
def addobjects(banana_file_loc, tomato_file_loc, banana_tex_loc, fruit_types = [1], num = 1, fruit_stat = [1], render_gt = False):
    if render_gt ==  False:
        for i in range(0,num):
            if (fruit_type[i] == 1):
                ...
                imported_object = bpy.ops.import_scene.obj(filepath=tomato_file_loc + str(int(fruit_stat[i])) + '.obj')
                bpy.data.objects['tomato' + str(int(fruit_stat[i]))].location = [0.12, - 7.7, 2.4]
                bpy.data.objects['tomato' + str(int(fruit_stat[i]))].rotation_euler = [80 * math.pi / 180,
                                                                                       -8 * math.pi / 180,
                                                                                       -70 * math.pi / 180]
                bpy.data.objects['tomato' + str(int(fruit_stat[i]))].scale = [0.135, 0.135, 0.135]
                bpy.ops.material.new()
                bpy.data.materials['Material'].name = 'tomato' + str(int(fruit_stat[i]))
                bpy.data.objects['tomato' + str(int(fruit_stat[i]))].active_material = bpy.data.materials['tomato' + str(int(fruit_stat[i]))]
                bpy.data.materials['tomato' + str(int(fruit_stat[i]))].use_nodes = True
                nodetree = bpy.data.materials['tomato' + str(int(fruit_stat[i]))].node_tree
                nodes = bpy.data.materials['tomato' + str(int(fruit_stat[i]))].node_tree.nodes
                nodes.new('ShaderNodeBsdfPrincipled')
                nodes['Principled BSDF'].inputs['Specular'].default_value = 0.2
                nodes['Principled BSDF'].inputs[0].default_value = [1.0, 0.057, 0.036, 1.0]
                nodetree.links.new(nodes['Principled BSDF'].outputs['BSDF'], nodes['Material Output'].inputs['Surface'])
        ...
        if (fruit_type[i] == 1):
            ...
            if (fruit_stat[i] == 1):
                ...
                nodes['Emission'].inputs['Color'].default_value = [1.0, 0.0, 1.0, 1.0]
...
tomato_file_loc = '/mnt/sdb1/repositories/Sys_sw_engg17-18/Simulations/tomato/Tomato'
...
for num in range(num_imgs):
    ...
    fruit_type = np.zeros(num_fruits)
    for k in range(num_fruits):
        fruit_type[k] = random.randint(1, 2) # 1 - banana, 2 -  tomato
    bananas=np.where(fruit_type==1)
    tomatoes=np.where(fruit_type==2)
    nbananas = bananas[0].shape[0]
    ntomatoes = tomatoes[0].shape[0]
    banana_status = np.zeros(nbananas)
    tomato_status = np.zeros(ntomatoes)
    for k in range(nbananas):
        banana_status[k] = random.randint(1,banana_states)
    for k in range(ntomatoes):
        tomato_status[k] = random.randint(1,tomato_states)
    fruit_status[np.where(fruit_type == 1)] = banana_status
    fruit_status[np.where(fruit_type == 2)] = tomato_status
    addobjects(banana_file_loc, tomato_file_loc, banana_tex_loc, fruit_types= fruit_type, num=num_fruits, fruit_stat = fruit_status, render_gt = False)
    #bpy.data.scenes["Scene"].render.filepath = save_loc+'/render'+str(num + 1)+'.png'
    #bpy.ops.render.render( write_still=True)
    #addobjects(banana_file_loc, tomato_file_loc, banana_tex_loc, fruit_types= fruit_type, num=num_fruits, fruit_stat = fruit_status, render_gt = True)
